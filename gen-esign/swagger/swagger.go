...
func (o Operation) CommentLines(funcName string) []string {
	...
	if len(o.Tags) > 0 {
		comments = append(comments, "https://docs.docusign.com/esign/restapi/"+o.Service+"/"+o.Tags[0]+"/"+o.Method)
	}
	return comments
}

// CallPath removes the accountId prefix for the op path.  Allows
// for credential to fill in.
func (o Operation) CallPath() string {
...
func (o Operation) GoFuncName(overrides map[string]string) string {
	if s, ok := overrides[o.OperationID]; ok {
		return s
	}
		...
		if p.In == "body" {
			...
			if p.Schema == nil || p.Schema.Type > "" {
				...
				if def, ok := defMap[p.Schema.Ref]; ok {
					tyx = "*model." + ToGoName(def.Name)
				}
			}
			return &Payload{GoName: ToGoNameLC(p.Name), Type: tyx}
		}
	}
	return nil
}
